...
from .models import CifarCNN, create_model
...
COMPUTE_SOURCES = ("neuron_surgery.py", "neuron_surgery_study.py", "losses.py", "models.py")
...
def _training_provenance(value: dict) -> dict:
    """Compare train/validation identity while ignoring final test construction."""
    result = _json(value)
...
def _load_inputs(baseline: str | Path, seeds: tuple[int, ...]):
    ...
    if tuple(seeds) != DEFAULT_SEEDS:
        raise ValueError("This predeclared study requires student seeds 42, 43, and 44")
    teacher_config_path = baseline / "teacher_f100" / "config.json"
    teacher_checkpoint = baseline / "teacher_f100" / "student.pt"
    if not teacher_config_path.is_file() or not teacher_checkpoint.is_file():
        raise FileNotFoundError("Missing factor-100 teacher config or checkpoint")
    teacher_config = from_dict(_read(teacher_config_path))
    teacher_config.validate(require_teacher=False)
    if (teacher_config.data.source != "cifar10" or teacher_config.data.imbalance_factor != 100
            or teacher_config.data.split_seed != 2026 or teacher_config.student.name != "cifar_teacher"
            or teacher_config.distillation.method != "supervised" or teacher_config.train.seed != 41):
        raise ValueError("Teacher does not match the predeclared factor-100 CIFAR-10-LT recipe")

    students = {}
    teacher_sha256 = fingerprint(teacher_checkpoint)
    for seed in seeds:
        path = baseline / f"kd_f100_seed{seed}"
        ...
        config.validate()
        recipe = (config.data.source, config.data.imbalance_factor, config.data.split_seed,
                  config.student.name, config.distillation.method, config.distillation.temperature,
        ...
        expected = ("cifar10", 100.0, 2026, "cifar_student", "kd", 4.0, 0.5, 5, 0.0,
                    81, 128, 0.05, 0.9, 0.0005, seed, "none")
...
def _protocol(baseline: Path, output: Path, device: str, teacher_config,
    ...
    return _json({
        ...
        "target_selection": {"classes": list(TAIL_CLASSES), "hard_fraction": 0.2,
                             "policy": "all mistakes plus highest-NLL correct rows to each class quota"},
        "companions": {"count": 5, "metric": "cosine_distance",
                       "embedding": "global-average-pooled normalized stage3"},
        "localization": {"stages": ["stage2", "stage3"],
        ...
        "repair": {"channel_budgets": list(CHANNEL_BUDGETS),
                   "learning_rates": list(LEARNING_RATES), "epochs": 20,
                   "samples_per_stream_per_epoch": 640, "batch_size": 64,
                   "optimizer": "SGD", "momentum": 0.9, "weight_decay": 0.0,
                   "scheduler": "cosine", "temperature": 4.0,
                   "loss": "CE(target)+KD_T4(anchor,preservation)+0.25*stage2/stage3 feature preservation"},
        ...
        "student_kd": {"seeds": list(DEFAULT_SEEDS), "epochs": 81, "method": "kd",
        ...
        "test_policy": "Official test data are constructed only after teacher selection is persisted; evidence is exploratory because prior workspace studies examined test.",
...
def _validation_report(model, loader, device, classes) -> dict:
    labels, probability = collect_predictions(model, loader, device)
    return {**prediction_metrics(labels, probability, classes), **_tail_metrics(labels, probability)}
...
def _train_candidate(directory: Path, identity: dict, teacher_config, teacher_checkpoint: Path,
                     anchor: CifarCNN, dataset, labels, target_indices, preservation_indices,
                     channels, device, classes, validation_loader, localization_sha256,
                     budget: int, learning_rate: float) -> dict:
    ...
    candidate = create_model("cifar_teacher", 10)
    load_model_checkpoint(candidate, teacher_checkpoint,
                          metadata("cifar_teacher", classes, 32, "cifar10"))
    history, verification = train_repair_candidate(
        candidate, anchor, dataset, labels, target_indices, preservation_indices, channels,
        device, epochs=20, samples_per_epoch=640, batch_size=64, learning_rate=learning_rate,
        momentum=0.9, temperature=4.0, feature_weight=0.25, seed=REPAIR_SEED)
    validation = _validation_report(candidate, validation_loader, device, classes)
    checkpoint = {**metadata("cifar_teacher", classes, 32, "cifar10"), "kind": "inference",
                  "epoch": 19, "student": candidate.cpu().state_dict(),
...
def _quality_report(directory: Path, labels, probabilities, classes) -> dict:
    ...
    quality["extended"] = extended_prediction_metrics(labels, probabilities, classes,
                                                       include_curve=False)
    quality.update(_tail_metrics(labels, probabilities))
    directory.mkdir(parents=True, exist_ok=True)
...
def _render_report(comparison: dict) -> str:
    ...
    lines = ["# CIFAR-10-LT neuron surgery and KD", "",
             "AI-Lancet-inspired channel repair was selected on the long-tailed validation split before test data were loaded.", "",
    ...
    lines.extend(["", f"Mean tail-recall delta: **{aggregate['tail_recall_delta']['mean']:+.4f}**.",
                  ...
                  "The official test set was examined by earlier workspace studies, so this evidence is exploratory.", ""])
    return "\n".join(lines)
...
def run_neuron_surgery_study(baseline="runs/cifar10-lt-multiseed",
    ...
    seeds = tuple(seeds)
    baseline, teacher_config, teacher_checkpoint, students = _load_inputs(baseline, seeds)
    _data_available(teacher_config)
    directory = Path(output).resolve()
    protocol = _protocol(baseline, directory, device, teacher_config, teacher_checkpoint, students)
    if dry_run:
        return {"dry_run": True, "output": str(directory), "protocol": protocol,
                "student_run_count": len(seeds), "repair_candidate_count": 6}
    ...
    if tuple(diagnostic_data.provenance["train_per_class"]) != EXPECTED_TRAIN_COUNTS:
        raise ValueError("Factor-100 training counts changed")
    teacher = create_model("cifar_teacher", 10)
    teacher_state = load_model_checkpoint(
        teacher, teacher_checkpoint, metadata("cifar_teacher", diagnostic_data.classes, 32, "cifar10"))
    ...
    def targets_compute():
        targets = select_hard_tail_targets(records)
        counts = tuple(sum(row["true_label"] == label for row in targets) for label in TAIL_CLASSES)
    ...
    companions = _stage_json(
        ...
        lambda: build_companion_records(records, diagnostics["embeddings"], target_plan["targets"]))
    ...
    consensus = _stage_json(
        ...
        lambda: consensus_channel_ranking(scores, repetitions=20, seed=REPAIR_SEED,
                                          top_fraction=0.25, stability_threshold=0.7))
    ...
    causal_identity = {"protocol_sha256": protocol_sha256,
                       ...
                       "targets_sha256": fingerprint(directory / "targets.json")}
    causal = _stage_json(
        directory / "causal_ablation.json", causal_identity,
        lambda: causal_channel_validation(teacher, target_loader, preservation_loader, consensus,
                                          resolved_device, accuracy_guardrail=0.005))
    ...
    def localization_compute():
        ...
        budgets = [budget for budget in CHANNEL_BUDGETS if len(ranking) >= budget]
    ...
    if not localization["available_channel_budgets"]:
        comparison = {"status": "no_repair_selected",
                      "reason": "Fewer than two channels passed consensus and causal guardrails.",
                      "protocol_sha256": protocol_sha256}
    ...
    original_validation = _validation_report(teacher, diagnostic_data.val, resolved_device,
                                             diagnostic_data.classes)
    ...
    for seed, baseline_row in students.items():
        ...
        repaired = replace(config, name=f"kd_repaired_f100_seed{seed}", output_dir=str(directory),
    ...
    # Test construction begins only after teacher selection and every student run are fixed.
    test_data = build_data(teacher_config.data, replace(teacher_config.train, workers=0, device="cpu"),
                           include_test=True, diagnostic=True)
    if _training_provenance(test_data.provenance) != _training_provenance(diagnostic_data.provenance):
        raise ValueError("Final test construction changed train/validation provenance")
    teacher.cpu()
    repaired_teacher, repaired_state = _model(teacher_config, repaired_checkpoint, test_data.classes)
    ...
    for condition, model, checkpoint in (("original", teacher, teacher_checkpoint),
        ...
        identity = {"checkpoint_sha256": fingerprint(checkpoint), "condition": condition,
                    "test_policy": protocol["test_policy"], "data": test_data.provenance}
        y, p = _save_predictions(directory / "teacher_test" / condition / "predictions.npz",
                                 identity, model, test_data.test, test_data.classes)
        teacher_rows.append((condition, y, p,
                             _quality_report(directory / "teacher_test" / condition, y, p,
                                             test_data.classes)))
    if not np.array_equal(teacher_rows[0][1], teacher_rows[1][1]):
        raise ValueError("Teacher test labels differ")
    ...
    for seed in seeds:
        ...
        baseline_model, _ = _model(baseline_config, students[seed]["checkpoint"], test_data.classes)
        candidate_checkpoint = directory / repaired_configs[seed].name / "student.pt"
        candidate_model, _ = _model(repaired_configs[seed], candidate_checkpoint, test_data.classes)
        values = []
        for condition, model, checkpoint in (("baseline", baseline_model, students[seed]["checkpoint"]),
            ...
            identity = {"checkpoint_sha256": fingerprint(checkpoint), "seed": seed,
                        "condition": condition, "data": test_data.provenance}
            y, p = _save_predictions(directory / "student_test" / f"seed{seed}" / condition /
                                     "predictions.npz", identity, model, test_data.test,
                                     test_data.classes)
            values.append((y, p, _quality_report(directory / "student_test" / f"seed{seed}" /
                                                 condition, y, p, test_data.classes)))
        if not np.array_equal(values[0][0], values[1][0]):
            raise ValueError(f"Seed {seed} paired test labels differ")
    ...
    comparison = {"status": "complete", "protocol_sha256": protocol_sha256,
                  ...
                  "test_interpretation": "Exploratory because earlier workspace studies examined the official test set."}
    write_json(directory / "comparison.json", comparison)
    _freeze_text(directory / "report.md", _render_report(comparison))
    ...
    for seed in seeds:
        ...
        for condition in ("baseline", "candidate"):
            base = f"student_test/seed{seed}/{condition}"
